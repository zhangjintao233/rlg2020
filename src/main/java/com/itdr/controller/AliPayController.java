...
public class AliPayController {
    ...
    private String callback(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (!result) {
                ...
                sr = ServerResponse.defeatedRS(
                        ConstCode.ALiPayEnum.FAIL_DELETE.getDesc()
                );
                //数据转换成json数据格式
                //数据返回给浏览器
                return "FAILED";
            ...
            e.printStackTrace();
            //验签失败，返回错误信息
            sr = ServerResponse.defeatedRS(ConstCode.ALiPayEnum.FAIL_DELETE.getDesc());
        ...
        sr = aliPayService.Callback(params);
        // 防止支付宝成功情况下重复请求
        if (sr.isSuccess()){
            return "SUCCESS";
        }else {
    ...
    public ServerResponse selectOrderNo(HttpSession session,Long orderNo){
        ...
        return aliPayService.selectOrderNo(user,orderNo);
    }
