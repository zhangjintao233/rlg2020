...
public class AliPayServiceImpl implements AliPayService {
    ...
    private AlipayTradePrecreateResponse test_trade_precreate(Order order, List<OrderItem> orderItems) throws AlipayApiException {
        ...
        String str = JsonUtils.obj2String(ObjectUtils.getBizContent(order, orderItems));
    ...
    public ServerResponse pay(Users user, Long orderNo) {
        ...
        if (o == null) {
            return ServerResponse.defeatedRS(ConstCode.OrderEnum.NULL_ORDER.getDesc());
        }
        /*//判断订单和用户是否匹配
        if (o.getUid() != user.getId()) {
            return ServerResponse.defeatedRS(ConstCode.OrderEnum.MATE_ORDER.getDesc());
        }*/
        //根据订单号查询对应商品详情
        List<OrderItem> list = orderItemMapper.selectByOrderNoAndUserId(orderNo,user.getId());
        //调用支付宝接口获取支付二维码

        AlipayTradePrecreateResponse response = null;
        try {
            response = test_trade_precreate(o, list);
    ...
    public ServerResponse Callback(Map<String, String> map) {
        ...
        if (order == null) {
            //不是要付款的订单
            sr = ServerResponse.defeatedRS(ConstCode.ALiPayEnum.FAIL_ALI.getDesc());
            return sr;
        }
        ...
        payInfo.setPlatformNumder(tarde_no);
    ...
    @Override
    public ServerResponse selectOrderNo(Users user, Long orderNo) {
        // 判断参数是否合法
        if (orderNo == null || orderNo < 0) {
            return ServerResponse.defeatedRS(ConstCode.UNLAWFULNESS_ALL);
        }
